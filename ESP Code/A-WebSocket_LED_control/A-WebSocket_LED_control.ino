...
const char *ssid = "Lightbar"; // The name of the Wi-Fi network that will be created
const char *password = "lightbar";   // The password required to connect to it, leave blank for an open network
...
void startWiFi() {
  WiFi.mode(WIFI_AP);// Start a Wi-Fi access point, and try to connect to some given access points. Then wait for either an AP or STA connection
  WiFi.softAPConfig(local_IP, gateway, subnet); 
  WiFi.softAP(ssid, password);
  WiFi.begin();// Start the access point
  //Serial.print("Access Point \"");
  //Serial.print(ssid);
  //Serial.println("\" started\r\n");

  //wifiMulti.addAP("KBY6H", "CC54BBV32BG6PJHK");   // add Wi-Fi networks you want to connect to
...
String padLeft(String inpt, int len, String toAdd){
  String add = "";
  for(int i=0; i<len - inpt.length(); i++){
    add = add + toAdd;
  }
  return add + inpt;
}
